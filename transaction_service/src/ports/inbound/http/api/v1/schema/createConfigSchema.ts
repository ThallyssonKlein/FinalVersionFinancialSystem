...
const used_calculated_schema = joi.object({
    reimbursement: joi.number().required(),
    values_average: joi.number().required(),
    time: joi.object({
...
const display_schema = joi.object({
    monthly_total: joi.number().required(),
    next_buy_date: joi.date().optional(),
    source: joi.string().optional(),
    amount: joi.number().optional()
})

const use_schema = joi.object({
    category: joi.string().required(),
    subcategory: joi.string().required(),
    default_name: joi.string().required()
})

const find_pair_schema = joi.object({
    pair_name: joi.string().required()
})

const rule_schema = joi.object({
    contains: joi.alternatives(joi.string(), joi.array().items(joi.string())).required(),
    ...
    used_in_calculation: joi.string().optional(),
    property: joi.string().optional(),
    type: joi.string().valid('STRING', 'NUMBER', 'DATE').optional(),
    between: joi.object({
        value1: joi.alternatives(joi.date(), joi.number()).required(),
        value2: joi.alternatives(joi.date(), joi.number()).required()
    }).optional(),
...
export default joi.object({
    ...
    custom_name: joi.boolean().required(),
    user_token: joi.string().required(),
    used_calculated: used_calculated_schema.required(),
    display: display_schema.required(),
    use: use_schema.required(),
    find_pair: find_pair_schema.optional(),
    rules: joi.array().items(rule_schema).required()
})